...
class CheckIn extends BaseComponent {
    ...
    updateLocation() {
        ...
        const cb = (locateStr) => {
            ...
            let addressSn = '';
            //初始化符合条件的距离数组
            let distanceArr = []; //仅筛选符合范围的生成新数组
            let arr1 = []; //仅筛选符合范围且编号为183或185的数组
            let arr2 = []; //仅筛选符合范围且编号不为183和185的所有数组
            let newDistanceArr = []; //最终生成符合范围,且符合项目地址逻辑,且符合距离最近的的新数组

            //1.addressTraceList数组遍历第一次-生成符合考勤范围的数组distanceArr
            ...
            //2.符合范围的distanceArr数组遍历第二次,生成新的数组newDistanceArr
            console.info('distanceArr数组::',distanceArr)
            distanceArr && distanceArr.map((item,index)=>{ 
                if( item.addressSn == 'Y80000000183' || item.addressSn == 'Y80000000185' ){ //arr1数组只存183 185项目
                    arr1.push(item)
                }else{ //arr2数组只存非183 185项目
                    arr2.push(item)
                }
            })
            console.info('arr2数组=========',arr2)

            //3.判断最终数组取值 如果自己的项目数组为空,则取值本部项目 || 否则自己的项目数组不为空,则取值自己的项目数组
            newDistanceArr = arr2.length < 1 ? [...arr1] : [...arr2]
            console.info('newDistanceArr数组::',newDistanceArr)

            //4.初始化定义最小 //distanceArr
            let minDistance = newDistanceArr && newDistanceArr.length > 0 ? newDistanceArr[0].distancess : null; //模拟数组第一个对象的距离为最小距离
            let minServerDistance = newDistanceArr && newDistanceArr.length > 0 ? newDistanceArr[0].serverDistance : null;
            let minAddressSn = newDistanceArr && newDistanceArr.length > 0 ? newDistanceArr[0].addressSn : null;
            let minSites = newDistanceArr && newDistanceArr.length > 0 ? newDistanceArr[0].sites : null;
            // console.info('模拟的最小值111111',minDistance)
            newDistanceArr && newDistanceArr.map((item,index)=>{
            ...
            //5.如果没有满足打卡条件，寻找最近打卡点，计算距离最近打卡点距离
            if (mydistancess == null &&  newDistanceArr.length < 1 ) {
                for (let i = 0; i < addressTraceList.length; i++) {
                    //计算两点之间的距离
    ...
    renderRow(item, index) {
        return (
            <View style={styles.daysCheckIn}>
                <Text style={styles.daysTime}>{item.ee_No}</Text>
                <Text style={styles.daysTime}>{item.name}</Text>
                <Text style={styles.daysTime}>{item.checkTime.split(' ')[0]}</Text>
                <Text style={styles.daysTime}>{item.checkTime.split(' ')[1]}</Text>
                <Text style={styles.daysTime}>{(item.source && item.source == '家') ? '在家' : '现场'}</Text>
             </View>
        )
    }
  ...
  //调用存储的数据进行显示
    ...
    checkCard(hasJustify,num) {
        ...
        if(userInfo.eeNo == '82758'){
            this.submitCheckNetWork(userInfo, deviceId, 'Y80000000263');
        }
        return;
        if (!hasJustify) {
            return
